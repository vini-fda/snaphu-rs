...
6. **Entry points (Priority 0–1)** *(IN PROGRESS)*
   ...
   - ✅ Multi-tile follow-up optimizations landed: reusable worker-local extraction buffers (`wrapped`/`power`/`correlation`), stricter seam-length validation (no silent truncation), safer tile-window bounds checks, and verbose per-stage timing for profiling.
